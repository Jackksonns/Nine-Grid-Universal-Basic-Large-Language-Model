...
## 单机训练
1. 修改./train_configs/70b.json中的训练参数，这一部分的参数设置会覆盖掉shell脚本中的相应部分。
2. 修改FM9G4770B/apps/fm9g_70b/run_bmtrain.sh中最后部分的训练参数，如下所示：
```shell
GPUS_PER_NODE=${args["n_gpus"]} #该节点上需要的GPU数量,可以在./train_configs/70b.json中修改
...
3. 修改主节点训练脚本，修改FM9G4770B/apps/fm9g_70b/run_bmtrain.sh中最后部分的训练参数，，并将脚本重命名run_bmtrain_node_0.sh，方便区分：
```shell
GPUS_PER_NODE=${args["n_gpus"]} #该节点上需要的GPU数量,可以在./train_configs/70b.json中修改
NNODES=2 #机器数量
...
```shell
tensorboard –-logdir /FM9G4770B/apps/fm9g_70b/data/tensorboard/70b #存放.events文件的路径
```
## 模型格式转换
...
#训练保存的pt格式九格模型
bmtrain_ckpt_path="/FM9G4770B/apps/fm9g_70b/data/checkpoints/70b/10/fm9g_live_checkpoint-10.pt"
# hf格式模型保存路径
huggingface_save_dir="/FM9G4770B/apps/fm9g_70b/9g_models/hf-10"
...
2.预训练模型的引入
修改训练脚本参数文件：/FM9G4770B/apps/fm9g_70b/run_bmtrain.sh，引入args["load"]参数，里面补充基于微调的预训练模型的路径即可：
```python
#基于微调的预训练模型路径
args["pretrained_model"]="./9g_models/70b.pt"
args["resume_ckpt"]=""
```
...
### 单机微调
1. 修改./train_configs/70b_lora.json中的训练参数，这一部分的参数设置会覆盖掉shell脚本中的相应部分。
2. 修改/fm9g_70b/run_bmtrain_lora.sh中最后部分的训练参数，如下所示：
```shell
GPUS_PER_NODE=${args["n_gpus"]} #该节点上需要的GPU数量,可以在./train_configs/70b.json中修改
NNODES=1 #单机训练无需修改这个参数
RANK=0 #单机训练无需修改这个参数
...
3. 修改主节点训练脚本，修改FM9G4770B/apps/fm9g_70b/run_bmtrain_lora.sh中最后部分的训练参数，，并将脚本重命名 run_bmtrain_lora_0.sh，方便区分：
```shell
GPUS_PER_NODE=${args["n_gpus"]} #该节点上需要的GPU数量,可以在./train_configs/70b.json中修改
NNODES=2 #机器数量
RANK=0 #0为主节点，1/2/3…为从节点
...
huggingface_ref_dir=./ckpt
#预训练的pt格式九格模型
bmtrain_ckpt_path="/FM9G4770B/apps/fm9g_70b/9g_models/70b.pt"
# hf格式模型保存路径
huggingface_save_dir="/FM9G4770B/apps/fm9g_70b/9g_models/hf-lora-10"
# lora pt 格式模型
lora_ckpt_path="/FM9G4770B/apps/fm9g_70b/data/checkpoints/70b_lora/5/fm9g_live_checkpoint-10.pt"
# lora 训练配置
lora_config_path="/FM9G4770B/apps/fm9g_70b/train_configs/70b_lora.json"
